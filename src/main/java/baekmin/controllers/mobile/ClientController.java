package baekmin.controllers.mobile;

import baekmin.Service.*;
import baekmin.domain.*;
import baekmin.repositories.*;
import baekmin.viewmodel.*;
...
//      상점아이디
    final static String P_MID = "100min0000";
     ...
     * INIpay 환경 파일
     */
    String inipayHome = "/home/baekmin/INIpay4JAVA";
...
//            기존 등록여부 확인
        ...
        strResult = "<script>window.location = 'baekmin://billing';</script>";
...
//        회원 포인트 사용내역 등록
     ...
     *  - INIpayMobile_WEB_manual_v.4.06.pdf 17페이지 P_NEXT_URL에서 처리후 안드로이드(baekmin://result) 호출하면 해당주문건의 결재 여부 api를 확인하여 결재 성공/실패 화면 표시
     */
    /**
     *
     * @param P_TID : 인증거래번호
...
//        strResult = "<script>window.location = 'baekmin_fail://result_fail';</script>";
        if("Y".equals(strResult2)) {
            strResult = "<script>window.location = 'baekmin://result';</script>";
        }
...
//        회원 존재여부 체크
        ...
        /*최근주소
        Query selectQuery = entityManager.createQuery("SELECT new baekmin.viewmodel.MoOrderList(t.orCd, t.regiDt, t.orStatus, c.cdNm)" +
