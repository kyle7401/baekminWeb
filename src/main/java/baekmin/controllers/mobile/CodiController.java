...
import baekmin.viewmodel.MoOrder4;
import baekmin.viewmodel.MoPromotion;
...
//        필수 컬럼 체크
        ...
        boolean equal = baekmin.utils.PasswordCrypto.getInstance().equal(tbEmployee.getCurPass(), encPwd);

        if(!equal)  return new ResponseEntity<>("현재 비밀번호가 일치하지 않습니다!", HttpStatus.BAD_REQUEST);

        String newEncPwd = baekmin.utils.PasswordCrypto.getInstance().encrypt(tbEmployee.getNewPass());
...
//        String encPwd = baekmin.utils.PasswordCrypto.getInstance().encrypt(epPass);
        String encPwd = tbEmployee.getEpPass();
        boolean equal = baekmin.utils.PasswordCrypto.getInstance().equal(epPass, encPwd);
