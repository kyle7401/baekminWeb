...
//        수거중
        if("0102".equals(tbOrder.getOrStatus())) {
            strText = "30분내로 백민코디가 방문예정입니다. 맡기실 옷들을 확인해주세요";
        ...
        if(StringUtils.isEmpty(strText)) return new ResponseEntity<>("주문상태가 주문접수(수거중) 또는 백민입고(배송중이) 아닙니다!", HttpStatus.BAD_REQUEST);
...
//        푸시전송
    ...
    public ResponseEntity<?> sendCodiModify(TbOrder tbOrder) throws IOException, GcmMulticastLimitExceededException, IllegalAccessException, NoSuchMethodException, InvocationTargetException {
        ...
        if(StringUtils.isEmpty(epCd) || epCd <= 0) return new ResponseEntity<>("해당 주문에 수거 코디가 배정되지 않았습니다!", HttpStatus.BAD_REQUEST);

        MoPush moPush = new MoPush();

        moPush.setPushGubun(3);
        moPush.setPushTitle("고객님의 주문에 담당코디가 재배정되었습니다.");
        moPush.setPushBody("고객님의 주문에 담당코디가 재배정되었습니다.");

        TbEmployee tbEmployee = tbEmployeeService.getTbEmployeeById(epCd);
        if(tbEmployee == null) return new ResponseEntity<>("해당 수거 코디 정보를 찾을수 없습니다!", HttpStatus.BAD_REQUEST);
