...
import baekmin.utils.SynapseCM;
...
//            기존에는 고객에게 SMS로 전송, 푸쉬 메세지 전송으로 대체?
    ...
    public MoOrderDetail2 getOrderDetail3(final Integer mbCd, final Long orCd) {
        TypedQuery selectQuery = entityManager.createQuery("SELECT new baekmin.viewmodel.MoOrderDetail2(" +
...
//        TypedQuery selectQuery = entityManager.createQuery("SELECT new baekmin.viewmodel.MoOrderDetail1(" +
...
//        코디가 배정되지 않으면 데이터를 찾지 못함
    ...
    public List<?> getCompleteList(final Integer mbCd, final List<String> status) {
        ...
        Query selectQuery = entityManager.createQuery("SELECT new baekmin.viewmodel.MoOrderList(t.orCd, t.regiDt, t.orStatus, c.cdNm, t.orChargeType)" +
